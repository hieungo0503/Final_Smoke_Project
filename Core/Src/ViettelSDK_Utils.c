...
char* castToString(union Value data, enum ValueType value_type)
{
	...
	{
		sprintf(cast_to_string_buffer, "%lu", *(uint32_t*) data.uint32_value);
	}
...
{
	uint8_t result = 0;
	for (int i = 0; binaryValue[i] != '\0'; i++) {
	        // Shift the current result to the left by one bit
	        result <<= 1;
	        // If the current character is '1', set the least significant bit of the result
	        if (binaryValue[i] == '1') {
	            result |= 1;
	        }
	    }
